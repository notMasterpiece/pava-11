            ...
            bcrypt.compare(password, user.password)
                .then(isMatch => {
                    if (isMatch) {
                        ...
                        // User Matched
                        jwt.sign(payload, process.env.MY_SECRET, {expiresIn: 36000}, (err, token) => {
                            if (err) throw err;
                            res.json({token})
                        });
                    } else {
...
router.get('/login/facebook', addSocketIdtoSession, passport.authenticate('facebook'));

router.get('/login/facebook/callback', passport.authenticate('facebook'), req => {
